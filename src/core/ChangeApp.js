...
export class ChangeApp extends EventEmitter {
  ...
  async initialize() {
    try {
      ...
      // REMOVED: No longer initializing founding proposal quiz on startup
  ...
  async attemptQuiz(username, quizId, userAnswers) {
    const answersArray = Object.values(userAnswers);
    const quiz = this.storage.get(`quiz_${quizId}`);
    ...
    for (let i = 0; i < quiz.questions.length; i++) {
      if (quiz.questions[i].correct === String.fromCharCode(65 + answersArray[i])) {
        correctCount++;
      }
    }
    ...
    await this._appendEvent({ type: eventType, data: { username, quizId, score } });
